    ...
    try {
      ...
      if (!data.taskId) {
        ...
        if (data.videos) {
          ...
          hideTiktokProgress();
          showToast("解析失败: 未返回 taskId", true);
        }
        return;
      }
...
async function handlePresetAdd() {
  ...
  const { introConfig, outroConfig, musicConfig, dedup, refLang, resourceTypes, outfitGuide, outfitSource, outfitPickUrl, outfitCallbackUrl, outfitPickIndex, outfitCallbackIndex, compress } = collectPresetConfig();
  ...
  try {
    const created = await request("/api/ai-presets", {
      method: "POST",
      body: JSON.stringify({ name, prompt, isDefault: presetModalEl.isDefault.checked, introConfig, outroConfig, musicConfig, dedup, refLang, resourceTypes, outfitGuide, outfitSource, outfitPickUrl, outfitCallbackUrl, outfitPickIndex, outfitCallbackIndex, compress }),
    });
    // 绑定暂存的变量文件
...
const autoPublish = {
  ...
  // ---- 手动触发监控 ----
  async triggerMonitor(matrixId) {
    try {
      showToast('正在触发监控…');
      await request(`/api/auto-publish/monitor/${encodeURIComponent(matrixId)}`, {
        method: 'POST',
      });
      showToast('监控已触发');
