...
const STAGE_NAMES = ["GREETING", "AUTH", "CONNECT", "TLS", "HTTP"];
...
export function checkIpGeoViaSocks5({
  host,
  port,
  username = "",
  password = "",
  ...
  return new Promise((resolve, reject) => {
    ...
    const targetPort = 80;
    log(`连接代理 ${host}:${port}（认证：${username ? "有" : "无"}）→ 目标 ${targetHost}:${targetPort}（HTTP geo 查询）`);
    const socket = net.createConnection({ host, port: Number(port) });
    let stage = STAGE_GREETING;
    let buffer = Buffer.alloc(0);
    let httpBuffer = "";
    ...
    const timer = setTimeout(() => {
      log(`geo 检测超时（${timeoutMs}ms），当前阶段：${STAGE_NAMES[stage]}`);
      socket.destroy();
      done(reject, new Error(`SOCKS5 geo 检测超时（阶段：${STAGE_NAMES[stage]}）`));
    }, timeoutMs);
    ...
    const parseGeoResponse = (raw) => {
      const sep = raw.indexOf("\r\n\r\n");
      if (sep === -1) {
        log(`geo 响应无 HTTP 头分隔符，原始内容前200字符：${raw.substring(0, 200)}`);
        return null;
      }
      let body = raw.substring(sep + 4).trim();
      log(`geo 响应体：${body.substring(0, 300)}`);
      if (/^[0-9a-fA-F]+\r\n/.test(body)) {
        const lines = body.split("\r\n");
        let decoded = "";
        for (let i = 0; i < lines.length; i += 2) {
      ...
      try {
        ...
        if (data.status !== "success") {
          log(`ip-api.com 返回失败：${data.message || data.status}`);
          return null;
        }
        return {
          ip: data.query || null,
        ...
        log(`geo JSON 解析失败：${e.message}，body=${body.substring(0, 200)}`);
        return null;
      }
    };
    ...
    socket.on("data", (chunk) => {
      ...
      if (stage === STAGE_GREETING) {
        ...
        log("CONNECT 成功，发送 HTTP geo 查询请求");
        stage = STAGE_HTTP;
        const req = `GET /json/?fields=status,message,query,city,zip,regionName,country,countryCode HTTP/1.1\r\nHost: ${targetHost}\r\nConnection: close\r\n\r\n`;
        socket.write(req);
      }
    });
    ...
    socket.on("close", () => {
      if (resolved) return;
      clearTimeout(timer);
      if (stage === STAGE_HTTP && httpBuffer) {
        log(`收到 HTTP 响应，长度=${httpBuffer.length} 字节`);
        const geo = parseGeoResponse(httpBuffer);
        if (geo) {
          log(`geo 解析成功：IP=${geo.ip}，城市=${geo.city}，邮编=${geo.zip}，地区=${geo.region}，国家=${geo.country}`);
          done(resolve, geo);
        } else {
          log("geo 解析失败，返回 null");
          done(resolve, null);
        }
      } else {
        log(`连接关闭（阶段 ${STAGE_NAMES[stage]}）`);
        done(reject, new Error(`代理连接在 ${STAGE_NAMES[stage]} 阶段被关闭`));
      }
    });
  });
