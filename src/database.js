...
export class LocalDatabase {
  ...
  #migrate() {
    ...
    this.#ensureColumn("remix_tasks", "multi_video_mode", "INTEGER DEFAULT 0");
    // 穿搭图库索引
    this.db.exec(`
    ...
    if (needsSeq.c > 0) {
      const rows = this.db.prepare("SELECT id FROM remix_tasks ORDER BY created_at ASC").all();
      rows.forEach((r, i) => {
        this.db.prepare("UPDATE remix_tasks SET seq_num = ? WHERE id = ?").run(i + 1, r.id);
      });
    }
    ...
    this.#ensureColumn("ai_remix_presets", "outfit_source", "TEXT"); // local or remote
    this.#ensureColumn("ai_remix_presets", "outfit_pick_url", "TEXT");
  ...
  updateAiRemixPreset(id, { name, prompt, isDefault, introConfig, outroConfig, musicConfig, dedup, refLang, resourceTypes, outfitGuide, outfitSource, outfitPickUrl, outfitCallbackUrl, outfitPickIndex, outfitCallbackIndex, compress }) {
    ...
    this.db.prepare(`
      ...
      SET name = COALESCE(?, name),
          ...
          outfit_source = COALESCE(?, outfit_source),
          outfit_pick_url = COALESCE(?, outfit_pick_url),
  ...
  upsertRemixVideoBySourceUrl({ creatorId, sourceUrl, title = null, thumbUrl = null, duration = null, createTime = null }) {
    const existing = this.db.prepare("SELECT id, create_time FROM remix_videos WHERE source_url = ?").get(sourceUrl);
    if (existing) {
      // 更新信息（如果有新数据）
      const sets = [];
      const params = [];
      ...
      // 如果已有记录没有 create_time 且新数据有，则更新
      if (createTime && !existing.create_time) { sets.push("create_time = ?"); params.push(String(createTime)); }
      if (sets.length) {
    ...
    const id = `rv_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;
    const ts = nowIso();
    this.db.prepare(`INSERT INTO remix_videos (id, creator_id, url, title, source_url, thumb_url, duration, downloaded, create_time, created_at) VALUES (?, ?, '', ?, ?, ?, ?, 0, ?, ?)`).run(id, creatorId, title, sourceUrl, thumbUrl, duration, createTime ? String(createTime) : null, ts);
    return this.getRemixVideo(id);
  }

  listRemixVideos(creatorId) {
    const rows = this.db.prepare(`SELECT * FROM remix_videos WHERE creator_id = ? ORDER BY COALESCE(create_time, '0') DESC, created_at DESC`).all(creatorId);
    return rows.map((r) => ({
  ...
  // --- Remix 任务管理 ---
  createRemixTask({ title, mode, videoUrls, sourceVideos = null, ratio = "9:16", creatorId = null, matrixIds = null, presetId = null, prompt = null, introEnabled = true, outroEnabled = true, musicEnabled = true, introId = null, outroId = null, musicId = null, cdpInstanceId = null, multiVideoMode = false }) {
    const id = `rt_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;
    const ts = nowIso();
    const maxSeq = this.db.prepare("SELECT COALESCE(MAX(seq_num), 0) as m FROM remix_tasks").get();
    const seqNum = maxSeq.m + 1;
    this.db.prepare(`
      INSERT INTO remix_tasks (id, seq_num, title, status, mode, video_urls_json, source_videos_json, video_count, ratio, created_at, creator_id, matrix_ids_json, preset_id, prompt, intro_enabled, outro_enabled, music_enabled, intro_id, outro_id, music_id, cdp_instance_id, multi_video_mode)
      VALUES (?, ?, ?, 'PENDING', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, seqNum, title, mode, JSON.stringify(videoUrls), sourceVideos ? JSON.stringify(sourceVideos) : null, videoUrls.length, ratio, ts, creatorId, matrixIds ? JSON.stringify(matrixIds) : null, presetId, prompt, introEnabled ? 1 : 0, outroEnabled ? 1 : 0, musicEnabled ? 1 : 0, introId, outroId, musicId, cdpInstanceId, multiVideoMode ? 1 : 0);
  ...
  #mapRemixTask(r) {
    return {
      id: r.id, seqNum: r.seq_num, title: r.title, status: r.status, mode: r.mode,
      ...
      multiVideoMode: r.multi_video_mode === 1,
